...
class DAGWriter:
    ...
    dag_write_folder = DAGS_FOLDER + '/AirflowDags/'
    ...
    def write_dag(cls, template_id, jinja_file, dag_id,  template_data):
        ...
        try:
            ...
            with dag_write_path.open(mode='w') as fp:
                fp.write(rendered)
                repo = git.Repo(os.path.dirname(os.path.abspath(dag_write_path)))
                dag_filename = os.path.basename(dag_write_path)
                # Provide a list of the files to stage
                repo.index.add([dag_filename])
                # Provide a commit message
                repo.index.commit('Template created: ' + dag_filename.split('.')[0])
    ...
    def delete_dag(cls, template_id, dag_id, session=None):
        try:
            ...
            if dag_path.exists():
                os.remove(dag_path)
                repo = git.Repo(os.path.dirname(os.path.abspath(dag_path)))
                dag_filename = os.path.basename(dag_path)
                repo.index.remove([dag_filename],working_tree = True)
                # Provide a commit message
                repo.index.commit('Deleting DAG: ' + dag_filename.split('.')[0])
                repo.git.push('origin', 'master')
